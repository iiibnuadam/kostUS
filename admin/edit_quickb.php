...
if (!isset($_GET['id_book'])) {
    echo "<script>window.location.href='index.php?page=manage_quickb'; alert('Silahkan pilih User yang mau di edit terlebih dahulu !');</script>";
}
$id_book = trim(strval($_GET['id_book']));

$sqla = "SELECT * FROM `books` WHERE `id_book` = '$id_book'";
$resulta = $connect->query($sqla);

if ($resulta->num_rows > 0) {
    while ($row = $resulta->fetch_assoc()) {
        ?>
        <form action="../backend/process.php?op=editquickb" method="post">
            <div class="form-group row">
                <label for="staticEmail" class="col-sm-2 col-form-label">ID Book</label>
                <div class="col-sm-10">
                    <input type="text" class="form-control" id="id_book" name="id_book" value="<?php echo $row['id_book']; ?>" readonly>
                </div>
            </div>
            <div class="form-group row">
